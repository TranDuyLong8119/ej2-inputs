...
let Uploader = class Uploader extends Component {
    ...
    uploadInProgress(e, files, customUI, request) {
        ...
        if (!isNullOrUndefined(li)) {
            ...
            let iconEle = li.querySelector('.' + CLEAR_ICON);
            if (!isNullOrUndefined(iconEle)) {
    ...
    removecanceledFile(e, file) {
        ...
        let spinnerTarget = liElement.querySelector('.' + CLEAR_ICON);
        if (!isNullOrUndefined(liElement)) {
            hideSpinner(spinnerTarget);
            detach(liElement.querySelector('.e-spinner-pane'));
        }
    ...
    renderFailureState(e, file, liElement) {
        ...
        /* istanbul ignore next */
        retryElement.addEventListener('click', (e) => { this.reloadcanceledFile(e, file, liElement); }, false);
    }
    reloadcanceledFile(e, file, liElement) {
        file.statusCode = '1';
    ...
    chunkUploadInProgress(e, metaData, custom) {
        ...
        if (!isNullOrUndefined(liElement)) {
            if (!(liElement.querySelectorAll('.' + PROGRESS_WRAPPER).length > 0)) {
                ...
                let clearIcon = liElement.querySelector('.' + CLEAR_ICON) ? liElement.querySelector('.' + CLEAR_ICON) :
     ...
     * @returns void
     */
    retry(fileData, fromcanceledStage) {
        let fileDataFiles = this.validateFileType(fileData);
        this.retryFailedFiles(fileDataFiles, fromcanceledStage);
    }
    retryFailedFiles(fileData, fromcanceledStage) {
        let files = this.getFiles(fileData);
        for (let i = 0; i < files.length; i++) {
            if (files[i].statusCode === '5' || files[i].statusCode === '0') {
                if (this.asyncSettings.chunkSize > 0) {
                    ...
                    let liElement = this.fileList[this.filesData.indexOf(files[i])];
    ...
    cancelUpload(fileData) {
        ...
        for (let i = 0; i < files.length; i++) {
            if (files[i].statusCode === '3') {
                ...
                this.updateMetaData(metaData);
                let liElement = this.getLiElement(files[i]);
...
let ColorPicker = class ColorPicker extends Component {
    ...
    initWrapper() {
        let wrapper = createElement('div', { className: 'e-' + this.getModuleName() + '-wrapper' });
    ...
    createSplitBtn() {
        let splitButton = createElement('button', { className: 'e-split-colorpicker' });
        ...
        let preview = createElement('span', { className: SPLITPREVIEW });
    ...
    createPalette() {
        ...
        if (this.presetColors) {
            let paletteGroup = createElement('div', { className: 'e-custom-palette' });
    ...
    appendPalette(colors, key, refEle) {
        let palette = createElement('div', { className: PALETTES, attrs: { 'tabindex': '0' } });
        if (refEle) {
    ...
    createPicker() {
        ...
        let hsvContainer = createElement('div', { className: HSVCONTAINER });
        this.appendElement(hsvContainer);
        hsvContainer.appendChild(createElement('div', { className: HSVAREA }));
        let dragHandler = createElement('span', { className: HANDLER, attrs: { 'tabindex': '0' } });
    ...
    createSlider() {
        let sliderPreviewWrapper = createElement('div', { className: 'e-slider-preview' });
        this.appendElement(sliderPreviewWrapper, 1);
        this.createPreview(sliderPreviewWrapper);
        let sliderWrapper = createElement('div', { className: 'e-colorpicker-slider' });
        sliderPreviewWrapper.insertBefore(sliderWrapper, sliderPreviewWrapper.children[0]);
        let slider = createElement('div', { className: 'e-hue-slider' });
        ...
        let opacityBgTrack = createElement('div', { className: 'e-opacity-empty-track' });
    ...
    createPreview(parentEle) {
        let previewContainer = createElement('div', { className: PREVIEW });
        parentEle.appendChild(previewContainer);
        let preview = createElement('span', { className: 'e-preview ' + CURRENT });
    ...
    getNumericTextBoxModel(value, label, max) {
        return {
            value: value,
            placeholder: label,
            min: 0,
    ...
    createInput(child = 2) {
        let inputWrap = createElement('div', { className: INPUTWRAPPER });
        this.appendElement(inputWrap, child);
        let container = createElement('div', { className: 'e-input-container' });
    ...
    createCtrlBtn() {
        if (this.modeSwitcher || this.showButtons) {
            this.l10n.setLocale(this.locale);
            let btnWrapper = createElement('div', { className: CTRLSWITCH });
            this.container.appendChild(btnWrapper);
            if (this.showButtons) {
                let controlBtnWrapper = createElement('div', { className: CTRLBTN });
    ...
    appendModeSwitchBtn() {
        let modeSwitcher = createElement('button', {
            ...
            switch (prop) {
                ...
                case 'cssClass':
                    if (this.isPicker()) {
                        ...
                        newProp.cssClass.split(' ').forEach((cls) => {
                            if (cls === 'e-hide-value') {
                                remove(select('.' + INPUTWRAPPER, this.container));
                            }
                        });
