...
function formSelectedData ( selectedFiles : FileInfo ) : void {
    ...
    fileSize.innerHTML = proxy.formatSize(selectedFiles.size);
...
function removeFiles(args : any) : void {
    let index: number = proxy.fileList.indexOf(args.currentTarget.parentElement);
    proxy.remove(proxy.filesData[index]);
}